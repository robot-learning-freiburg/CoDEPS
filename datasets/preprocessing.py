...
def prepare_for_network(output: Dict[str, Any], cfg: CN):
    ...
    for offset, img in output['rgb'].items():
        output['rgb'][offset] = ToTensor()(img)
        if cfg.active:
            output['rgb'][offset] = Normalize(mean=cfg.rgb_mean,
                                              std=cfg.rgb_std)(output['rgb'][offset])
...
def augment_data(output: Dict[str, Any], cfg: CN):
    ...
    for key, value in output.items():
        if key == 'rgb':
            for offset, img in value.items():
                if do_flip:
                    img = img.transpose(Image.FLIP_LEFT_RIGHT)
                output[key][offset] = color_augmentation(img)
        elif key == 'camera_model':
            if do_flip:
                # Flip principal point horizontally
                img_width = output['rgb'][0].size[0]
                cx = img_width - value[2] - 1
                output[key][2] = cx
        elif do_flip:
