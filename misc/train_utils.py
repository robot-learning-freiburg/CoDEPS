...
def init_logging(args, log_dir, run_dir, config, init_wandb=False):
    ...
    if init_wandb:
        # wandb_summary = wandb.init(project="ondap_research",
        wandb_summary = wandb.init(project="paper_results",
                                   entity="online_domain_adaptation",
                                   dir=run_dir,
...
def gen_dataloader(args, cfg, rank, world_size):
    ...
    if args.eval:
        ...
        if cfg.dataset.name == "kitti_360":
            dataset_train = Kitti360(cfg.dataset.train_split, cfg.dataset, return_depth=True,
            ...
            dataset_train = Cityscapes(cfg.dataset.train_split, cfg.dataset, return_depth=True,
                                       return_only_rgb=cfg.dataset.return_only_rgb,
                                       label_mode=cfg.dataset.label_mode)
        elif cfg.dataset.name == "sem_kitti_dvps":
            dataset_train = SemKittiDvps(cfg.dataset.train_split, cfg.dataset, return_depth=True,
                                         sequences=cfg.dataset.train_sequences,
                                         label_mode=cfg.dataset.label_mode)
        else:
    ...
    if cfg.dataset.name == "kitti_360":
        dataset_val = Kitti360(cfg.dataset.val_split, cfg.dataset, return_depth=True,
                               sequences=cfg.dataset.val_sequences,
                               sequence_reference_mode="semantic",
                               label_mode=cfg.dataset.label_mode)
    elif cfg.dataset.name == "cityscapes":
        dataset_val = Cityscapes(cfg.dataset.val_split, cfg.dataset, return_depth=True,
                                 return_only_rgb=cfg.dataset.return_only_rgb,
                                 label_mode=cfg.dataset.label_mode)
    elif cfg.dataset.name == "sem_kitti_dvps":
        dataset_val = SemKittiDvps(cfg.dataset.val_split, cfg.dataset, return_depth=True,
                                   sequences=cfg.dataset.val_sequences,
                                   label_mode=cfg.dataset.label_mode)
    else:
        raise NotImplementedError(f"Dataset {cfg.dataset.name} is not yet implemented")
