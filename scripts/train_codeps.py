...
parser = argparse.ArgumentParser(description="Pretrain CoDEPS on a given dataset")
...
parser.add_argument("--pre_train", type=str, nargs="+",
                    help="Start from the given pre-trained snapshots, overwriting each with the "
                         ...
                         "'backbone_po_depth', 'depth_head', 'pose_head', 'flow_head', "
                         "'semantic_head', 'instance_head', or 'semantic_head'. In that case only "
                         "that part of the network will be loaded from the snapshot")
...
def train(model, optimizer, scheduler, dataloader, meters, config, epoch, device,
    ...
    for batch_idx, sample in enumerate(dataloader):
        ...
        # Run network
        optimizer.zero_grad()
        losses, results, stats = model(sample_cuda, "train", depth_offsets=rgb_frame_offsets)
        if not debug:
        ...
        # ToDo: Why did we put this in here before?
    ...
    if log_train_samples:
        ...
        for batch_idx, sample in enumerate(log_dataloader):
            with torch.no_grad():
                ...
                _, results, _ = model(sample_cuda, "eval",
                                      depth_offsets=rgb_frame_offsets,
...
def validate(model, dataloader, config, epoch, wandb_summary, device, global_step,
    ...
    if distributed.is_initialized() and distributed.get_world_size() > 1:
        # Otherwise, not all GPUs will have the same keys in their stats dictionaries
        do_class_wise_depth_stats = False
    log_step_interval = config.logging.log_step_interval
    ...
    if do_class_wise_depth_stats:
        for i in range(dataloader.dataset.num_classes):
            val_meters["metrics"][f"d_abs_rel_c{i}"] = AverageMeter(())
            val_meters["metrics"][f"d_a1_c{i}"] = AverageMeter(())
            val_meters["metrics"][f"d_a2_c{i}"] = AverageMeter(())
            val_meters["metrics"][f"d_a3_c{i}"] = AverageMeter(())
            val_meters["metrics"][f"d_rmse_c{i}"] = AverageMeter(())
            val_meters["metrics"][f"d_rmse_log_c{i}"] = AverageMeter(())
            val_meters["metrics"][f"d_sq_rel_c{i}"] = AverageMeter(())
    ...
    for it, sample in tqdm(enumerate(dataloader), disable=not print_results, total=len(dataloader),
        ...
        with torch.no_grad():
            ...
            losses, results, stats = model(sample_cuda, "train",
                                           depth_offsets=rgb_frame_offsets,
                                           do_panoptic_fusion=do_panoptic_fusion,
                                           do_class_wise_depth_stats=do_class_wise_depth_stats,
    ...
    if wandb_summary is not None:
        ...
        logging.log_wandb(wandb_summary,
                          ...
                          learning_rate=None,
                          global_step=global_step)
        logging.log_wandb_depth_class_v2(wandb_summary, wandb_panel, val_meters["metrics"], False,
...
def main(args):
    ...
    # Load configuration
    config = io_utils.gen_config(args)
    ...
    momentum = 1. - 1. / len(train_dataloader) # Compute a running mean
    train_meters = {
        "losses": {
            "loss": AverageMeter((), momentum),
            "depth_recon": AverageMeter((), momentum),
            "depth_smth": AverageMeter((), momentum),
            "flow_smth": AverageMeter((), momentum),
            "flow_sparsity": AverageMeter((), momentum),
            "semantic": AverageMeter((), momentum),
            "center": AverageMeter((), momentum),
            "offset": AverageMeter((), momentum),
            "thing_mask": AverageMeter((), momentum)
        },
        "metrics": {
            # Depth
            "d_abs_rel": AverageMeter(()),
